...
- 生产 API Worker 使用 Workers Free 免费套餐：全部统计汇总已合并为单 Cron（北京时间 01:00），受 5 个 Cron 账号上限与 10ms CPU 限制约束，不要在 wrangler.jsonc 中新增 Cron 或恢复历史上的多 Cron 形态；付费套餐的恢复方式见 analytics/README.md。
