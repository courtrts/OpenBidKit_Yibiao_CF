...
// 免费套餐适配：部署到自己的 Cloudflare 账户后，把这里改成你的 API Worker 实际地址
// （即 wrangler 部署 agnet-analytics-api 后输出的 https://<worker名>.<账号>.workers.dev 或自定义域名）。
// 非本地打开 dashboard 时，API base 只允许“同域”或下面这个 productionApiBase。
const productionApiBase = 'https://toubiao.ztok.dpdns.org';
