...
export async function loadModelInfoCache(options = {}) {
  ...
  void loadModelInfoSource().catch((error) => setModelInfoCacheStatus(error?.message || String(error), 'error'));
  if (!options.quiet) {
...
const MODEL_INFO_SOURCE_ORIGIN_LABEL = {
  admin: '管理端保存',
  env: '环境变量',
  default: '内置默认',
};

// 读取当前生效的模型目录源（管理端保存 > 环境变量 > 内置默认）。
async function loadModelInfoSource() {
  const data = await requestJson('/api/model-info-cache/source');
  state.modelInfoSourceUrl.value = data.sourceUrl || '';
  state.modelInfoSourceOrigin.textContent = MODEL_INFO_SOURCE_ORIGIN_LABEL[data.origin] || data.origin || '-';
}

// 保存模型目录源；输入为空表示清除管理端配置并回退到环境变量或内置默认。
async function saveModelInfoSource() {
  const sourceUrl = state.modelInfoSourceUrl.value.trim();
  try {
    state.modelInfoSourceSaveButton.disabled = true;
    state.modelInfoSourceResetButton.disabled = true;
    setModelInfoCacheStatus(sourceUrl ? '正在保存模型目录源...' : '正在恢复默认模型目录源...', '');
    await requestJson('/api/model-info-cache/source', { method: 'POST', body: { sourceUrl } });
    await loadModelInfoSource();
    setModelInfoCacheStatus(sourceUrl ? '模型目录源已保存，点击"立即同步"生效。' : '模型目录源已恢复默认，点击"立即同步"生效。', 'ok');
  } catch (error) {
    setModelInfoCacheStatus(error?.message || String(error), 'error');
  } finally {
    state.modelInfoSourceSaveButton.disabled = false;
    state.modelInfoSourceResetButton.disabled = false;
  }
}
...
export function setupModelInfoCachePage() {
  ...
  state.modelInfoSourceSaveButton.addEventListener('click', saveModelInfoSource);
  state.modelInfoSourceResetButton.addEventListener('click', () => {
    state.modelInfoSourceUrl.value = '';
    void saveModelInfoSource();
  });
}
