...
// GitHub 仓库标识默认值（dashboard 仓库统计用）。自部署 fork 时可用环境变量
// GITHUB_REPO_FULL_NAME 覆盖为 "owner/repo"。
export const GITHUB_REPO_FULL_NAME = 'FB208/OpenBidKit_Yibiao';
export function githubRepoStatsCacheKey(fullName) {
  return `github_repo_stats:${String(fullName || GITHUB_REPO_FULL_NAME).trim()}`;
}
export const GITHUB_REPO_STATS_CACHE_TTL_SECONDS = 1800;
export const GITHUB_REPO_STATS_STALE_TTL_SECONDS = 604800;
// 模型目录数据源默认值（models.dev 目录格式）。运行时可配置，优先级：
// 管理端 KV 设置（model_info_cache:source_url）> 环境变量 MODEL_INFO_SOURCE_URL > 此默认值。
// 支持指向任意第三方或自部署的 models.dev 兼容目录。
export const DEFAULT_MODEL_INFO_SOURCE_URL = 'https://models.dev/api.json';
export const MODEL_INFO_SOURCE_URL_KEY = 'model_info_cache:source_url';
