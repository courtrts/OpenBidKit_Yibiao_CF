...
import {
  ...
  resolveModelInfoSourceUrl,
  saveModelInfoOverride,
  saveModelInfoSourceUrl,
  syncModelInfoCache,
} from '../services/modelInfoCache.js';
import { DEFAULT_MODEL_INFO_SOURCE_URL } from '../constants.js';
...
// 管理端读写模型目录源地址（支持任意第三方或自部署的 models.dev 兼容目录）。
// GET 返回当前生效值及来源（admin/env/default）；POST 保存新地址（sourceUrl 为空表示清除、回退）。
export async function handleAdminModelInfoSource(request, env) {
  if (!requireAdmin(request, env)) return unauthorized();
  if (!env.NOTICE_STORE) {
    return json({ code: 500, message: 'NOTICE_STORE is not configured' }, { status: 500 });
  }
  ...
  if (request.method === 'GET') {
    const resolved = await resolveModelInfoSourceUrl(env);
    return json({
      code: 0,
      sourceUrl: resolved.sourceUrl,
      origin: resolved.origin,
      default: DEFAULT_MODEL_INFO_SOURCE_URL,
    }, { headers: { 'Cache-Control': 'no-store' } });
  }
  ...
  if (request.method === 'POST') {
    ...
    try {
      const result = await saveModelInfoSourceUrl(env, sourceUrl);
      const resolved = await resolveModelInfoSourceUrl(env);
      return json({
        code: 0,
        ...result,
