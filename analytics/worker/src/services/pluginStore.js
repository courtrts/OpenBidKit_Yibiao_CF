...
// 插件包公网下载地址默认基地址（生产 R2 自定义域名）。
// 自部署时通过环境变量 PLUGIN_PACKAGE_PUBLIC_BASE_URL 覆盖为自己的 R2/CDN 地址。
const PLUGIN_PACKAGE_PUBLIC_BASE_URL = 'https://pub-e7a765184e924c72a1dbe429d9bf181c.r2.dev';
...
/** 生成客户端直接使用的插件包公网下载地址。公网基地址可通过环境变量
 *  PLUGIN_PACKAGE_PUBLIC_BASE_URL 覆盖（自部署时指向自己的 R2/CDN 公网地址）。 */
export function buildPluginPackageUrl(id, version, env) {
  const key = createPluginPackageKey(id, version);
  const encodedKey = key.split('/').map((segment) => encodeURIComponent(segment)).join('/');
  const base = String(env?.PLUGIN_PACKAGE_PUBLIC_BASE_URL || '').replace(/\/+$/, '') || PLUGIN_PACKAGE_PUBLIC_BASE_URL;
...
async function mirrorPluginPackage(env, resolved, existing) {
  ...
  const releaseUrl = buildPluginPackageUrl(resolved.id, resolved.version, env);
...
async function cleanupAllPluginPackagesLocked(env) {
  ...
  for (const row of rows.results || []) {
    if (normalizeText(row.release_url, PLUGIN_RELEASE_URL_MAX_LENGTH) === buildPluginPackageUrl(row.id, row.version, env)) {
    ...
    if (previousVersion
      && normalizeText(row.previous_release_url, PLUGIN_RELEASE_URL_MAX_LENGTH) === buildPluginPackageUrl(row.id, previousVersion, env)) {
...
export async function listPublicPlugins(env, options = {}) {
  ...
  return (result.results || [])
    ...
    .filter((plugin) => plugin && plugin.releaseUrl === buildPluginPackageUrl(plugin.id, plugin.version, env));
}
...
async function resolvePreviousRelease(env, existing, sameVersion, nextVersion) {
  ...
  if (!version || version === nextVersion || releaseUrl !== buildPluginPackageUrl(existing?.id, version, env)) {
...
async function publishResolvedPluginLocked(env, resolved, options = {}) {
  ...
  try {
    ...
    if (plugin.previousVersion
      && plugin.previousReleaseUrl === buildPluginPackageUrl(plugin.id, plugin.previousVersion, env)) {
