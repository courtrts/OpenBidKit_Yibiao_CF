...
// 归一化 MinerU 服务地址：仅接受 http/https，去掉末尾斜杠，非法值回退默认。
// 支持指向任意第三方或自部署的 MinerU 兼容服务。
function normalizeMineruBaseUrl(value, fallback) {
  ...
  return {
    file_parser: {
      ...
      mineru_base_url: normalizeMineruBaseUrl(
        ...
        defaultConfig.components.file_parser.mineru_base_url,
      ),
    },
