...
function resolveMineruBaseUrl(config) {
  const value = String(config?.components?.file_parser?.mineru_base_url || '').trim().replace(/\/+$/, '');
  return /^https?:\/\/[^\s/]/i.test(value) ? value : DEFAULT_MINERU_BASE_URL;
}
...
async function parseWithMineruAgent(filePath, options = {}) {
  ...
  const baseUrl = options.mineruBaseUrl || DEFAULT_MINERU_BASE_URL;
  ...
  const finalResult = await pollMineruAgent(taskId, fileName, baseUrl);
...
async function pollMineruAgent(taskId, fileName, baseUrl) {
...
async function parseWithMineruAccurate(filePath, token, options = {}) {
  ...
  const fileName = path.basename(filePath);
  const baseUrl = options.mineruBaseUrl || DEFAULT_MINERU_BASE_URL;
  const createResponse = await fetch(`${baseUrl}/api/v4/file-urls/batch`, {
    method: 'POST',
    headers: {
  ...
  await uploadFile(fileUrl, filePath);
  const finalResult = await pollMineruAccurate(token, batchId, fileName, baseUrl);
...
async function pollMineruAccurate(token, batchId, fileName, baseUrl) {
  const startedAt = Date.now();
...
async function parseDocumentWithConfig(app, filePath, config, options = {}) {
  ...
  const parseOptions = { preserveImages, assets, imageResolver: createImageResolver(assets), mineruBaseUrl: resolveMineruBaseUrl(config) };
  let markdown = '';
  try {
    if (provider === 'mineru-agent-api') {
