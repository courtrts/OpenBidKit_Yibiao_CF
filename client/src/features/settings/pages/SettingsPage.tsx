...
function normalizeComponentsState(components?: Partial<ComponentsConfig>): SettingsPageState['components'] {
  return {
    file_parser: {
      ...
      mineru_base_url: components?.file_parser?.mineru_base_url || 'https://mineru.net',
    },
...
function componentsFromState(components: SettingsPageState['components']): ComponentsConfig {
  return {
    file_parser: {
      ...
      mineru_token: components.file_parser.mineru_token || '',
      mineru_base_url: components.file_parser.mineru_base_url || '',
    },
    mermaid_concurrency_limit: normalizeComponentConcurrencyLimit(components.mermaid_concurrency_limit),
    html_concurrency_limit: normalizeComponentConcurrencyLimit(components.html_concurrency_limit),
  };
      ...
      {activeTab === 'components' && (
        <section className="settings-page-section">
          ...
          <div className="settings-list">
            ...
            {(state.components.file_parser.provider === 'mineru-accurate-api' || state.components.file_parser.provider === 'mineru-agent-api') && (
              <label className="settings-row">
                <div className="settings-row-copy">
                  <strong>MinerU 服务地址</strong>
                  <span>MinerU 兼容服务的根地址（http/https），可指向任意第三方或自部署服务；留空使用默认 https://mineru.net</span>
                </div>
                <input
                  type="text"
                  value={state.components.file_parser.mineru_base_url || ''}
                  ...
                  onChange={(event) => setState((prev) => ({
                    ...
                    components: {
                      ...
                      file_parser: {
                        ...prev.components.file_parser,
                        mineru_base_url: event.target.value,
                      },
                    },
                  }))}
                />
